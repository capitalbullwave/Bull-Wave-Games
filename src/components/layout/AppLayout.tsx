  ...
  return (
    <div className="min-h-screen w-full bg-white flex justify-center items-center overflow-hidden font-sans">
      ...
      <div className="w-full max-w-[450px] h-screen bg-[#fafafb] text-slate-800 flex flex-col relative shadow-[0_0_50px_rgba(0,0,0,0.15)] border-x border-glass-border overflow-hidden">
        ...
        <main className={`flex-1 overflow-y-auto scroll-smooth ${pathname === "/about" ? "pb-6" : "pb-24"}`}>
          ...
          <nav className="absolute bottom-0 left-0 right-0 h-16 bg-white border-t border-glass-border z-40 px-2 flex items-center justify-between shadow-[0_-5px_15px_rgba(0,0,0,0.05)]">
            ...
            <div className="relative -top-5 flex flex-col items-center">
              <button
                ...
                className="w-16 h-16 rounded-full bg-gradient-to-tr from-[#800000] to-[#D4AF37] p-1 shadow-[0_0_20px_rgba(212,175,55,0.6)] flex items-center justify-center hover:scale-105 active:scale-95 transition-transform"
              >
                <div className="w-full h-full rounded-full bg-white flex items-center justify-center overflow-hidden relative">
                  <Gift size={26} className="text-[#800000]" />
                </div>
              </button>
              <span className="text-[10px] font-black text-[#800000] mt-1 text-center bg-white border border-[#D4AF37] px-2 py-0.5 rounded-full shadow-sm relative z-10 whitespace-nowrap">
                Get ₹500
              </span>
            </div>
      ...
      {/* Floating Side Widgets (Visible only on larger screens, right edge) */}
      <div className="hidden sm:flex flex-col gap-3 fixed right-6 top-1/2 -translate-y-1/2 z-40">
        {[
          { label: "Bonus", icon: Gift, color: "bg-red-500", onClick: () => router.push("/rewards") },
          { label: "Support", icon: Headset, color: "bg-amber-500", onClick: () => router.push("/support") },
          { label: "VIP Club", icon: Trophy, color: "bg-purple-600", onClick: () => router.push("/leaderboard") },
          { label: "Community", icon: Users, color: "bg-sky-500", onClick: () => router.push("/referrals") },
        ].map((item, idx) => (
          <button
            key={idx}
            onClick={item.onClick}
            className={`w-12 h-12 rounded-full text-white ${item.color} shadow-lg flex items-center justify-center hover:scale-110 hover:-translate-x-1 active:scale-95 transition-all relative group`}
          >
            <item.icon size={20} />
            <span className="absolute right-14 bg-black/80 px-2 py-1 rounded text-xs whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none border border-glass-border">
              {item.label}
            </span>
          </button>
        ))}
